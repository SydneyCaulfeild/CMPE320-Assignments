...
void testJumble() {
	...
	// Test that the size of the array is correct with respect to the size of the word using jp4.
	cout << "\nCheck that the size of the array is equal to the size of the word times the size of the difficulty:";
	// "BYE" = 3 therefore 3*2 = 16
	if (sz == 6) {
		cout << "\nSize is correct.\n";
	}
	else {
	...
	// Test giving a word with an invalid length.
	cout << "\nShould get an error message saying that the word is too long:\n";
	try {
		JumblePuzzle jp5("toolongofaword", "easy");
	}
	catch (BadJumbleException& e){
		cout << e.what() << endl;
	}
	cout << "\nShould get an error message saying that the word is too short:\n";
	try {
		JumblePuzzle jp6("hi", "medium");
	}
	catch (BadJumbleException& e){
		cout << e.what() << endl;
	}

	// Test giving an invalid difficulty level.
	cout << "\nShould get an error message saying that the difficulty is invalid:\n";
	try {
		JumblePuzzle jp7("dog", "eaaasy");
	}
	catch (BadJumbleException& e){
		cout << e.what() << endl;
	}

	// Test that direction is a valid letter using jp4.
	cout << "\nTesting that the direction is valid.";
	if (jp4.getDirection() == 'n' || jp4.getDirection() == 'e' || jp4.getDirection() == 's' || jp4.getDirection() == 'w') {
		cout << "\nThe direction is valid. The direction is: " << jp4.getDirection() << endl;
	}
	else {
		cout << "\nThe direction is not valid as it is not n,s,e, or w.";
	}

	// Test that the row and column positions are within the array length using jp4.
	cout << "\nTesting that the row and column starting position indices are within the size of the array.";
	if ((jp4.getRowPos() < 6 && jp4.getRowPos() >= 0) && (jp4.getColPos() < 6 && jp4.getColPos() >= 0)) {
		cout << "\nThe row and column starting positions are within the size of the array.\nRow starting index: " << jp4.getRowPos() << "\nColumn starting index: " << jp4.getColPos() << "\nValid positions are 0-" <<jp4.getSize() <<endl;
	}
	else {
		cout << "\nThe row and/or column starting positions are not valid.";
	}
